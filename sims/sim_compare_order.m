function [xstar, iters] = sim_compare_order(test_nbr,config)

close all

iters = 0; 

relative_save_dir = strcat('test_',int2str(test_nbr));
...
funorder = config.funorder; %order followed for generating the functions from a class of functions 
record = config.record; %if record = 1, record the iterates
...
rng(s);
%bias = randn(n,1);
funcs =  listDeterQuad8(m,n,funorder);  
...
for j=1:nbr_paths
    tic
    if(j == 1) 
        [ave_iter(:,j), ave_step, bias(:,j), step,iters,~] = incr_grad(funcs,order,m,n,nCycles,step0,x,stoclevel,removebias,record,stepDecay);
    else 
        %%% the second parameter (average step) is the same, no need to save it again
        [ave_iter(:,j), ~, bias(:,j), step,iters,~] = incr_grad(funcs,order,m,n,nCycles,step0,x,stoclevel,removebias,stepDecay); 
    end
