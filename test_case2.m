function [xstar, iters] = test_case2(test_nbr,config)
...
for j=1:nbr_paths
    tic
    if(j == 1) 
        [ave_iter(:,j), ave_step, bias(:,j), step,iters] = incr_grad(funcs,order,m,n,nCycles,step0,x,stoclevel,removebias,record);
    else 
        %%% the second parameter (average step) is the same, no need to save it again
        [ave_iter(:,j), ~, bias(:,j), step,iters] = incr_grad(funcs,order,m,n,nCycles,step0,x,stoclevel,removebias); 
    end
